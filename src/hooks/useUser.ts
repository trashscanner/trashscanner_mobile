...
export const useUser = (autoFetch = true): UseUserResult => {
  ...
  const handleRequest = async (request: () => Promise<void>, shouldRefetch = true) => {
    setIsLoading(true);
    setError(null);
    try {
      await request();
      ...
      const axiosError = err as AxiosError<{ message: string }>;
      const errorMessage =
        axiosError.response?.data?.message || axiosError.message || 'Произошла ошибка';
  ...
  const deleteAvatar = () => handleRequest(() => userApi.deleteAvatar());
