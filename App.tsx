...
export type ScreenKey = 'auth' | 'camera' | 'analysis' | 'stats' | 'map' | 'profile';

const App: React.FC = () => {
  const [currentScreen, setCurrentScreen] = useState<ScreenKey>('auth');
  const [capturedImage, setCapturedImage] = useState<string | null>(null);

  const showNavigation = currentScreen !== 'auth' && currentScreen !== 'analysis';

  const activeTab = useMemo<BottomTabKey>(() => {
    if (currentScreen === 'stats' || currentScreen === 'map' || currentScreen === 'profile') {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={[styles.screenWrapper, showNavigation && styles.screenWithNav]}>
        {currentScreen === 'auth' && (
